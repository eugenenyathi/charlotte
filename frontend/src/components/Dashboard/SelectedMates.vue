<template>
  <section class="selected-mates-container" v-if="selectedMates.length !== 0">
    ...
    <BaseButton class="request-room-btn" text="Request Room" @btnFn="requestRoom" v-if="!isLoading" />
...
const requestRoom = async () => {
  if (editingSelection.value) return emit('show', 'preferredMates')
...
const requestFailed = (data) => {
  data.roommates.forEach((candidate) => removeStudent(candidate.id))

  showAlert(true, 'taken student(s) removed, please add new roommates.', 'danger')
  removeAlert()
}
</script>
